...
def get_char_profile(REALM, CHARACTER):
...
def get_char_media(REALM, CHARACTER):
  BLIZZARD_TOKEN = get_blizz_token()
...
async def on_message(message):
  ...
  if msg.startswith('m!commands') or msg.startswith('m!help'):
    ...
    char_media = get_char_media(REALM, CHARACTER)
    ...
    REALM = realm_char_query[0].replace('\'', '').replace(' ', '-')
    CHARACTER = realm_char_query[1]
    char_profile = get_char_profile(REALM, CHARACTER)
    char_media = get_char_media(REALM, CHARACTER)
